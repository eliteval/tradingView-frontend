...
$(function () {
        ...
        $(object)
            ...
            .each(function () {
                ...
                if (cardHeader.next().find("select").length != 0) {
                    ...
                    cardHeader.next().find("input").attr("type") == "checkbox"
                ) {
                    if (
                        cardHeader
                            .next()
                    ...
                    functionReady: function (origin, tooltip) {
                        ...
                        function justShowOne(object) {
                            ...
                            $(object)
                                ...
                                .attr(
                                    "rotate-icon",
                                    $(object).find("a").attr("rotate-icon") ===
                                        "true"
                                        ? "false"
                                        : "true"
                                );
                        ...
                        $(".time-input").on("input", function () {
                            if (
                                String($(this).val()).indexOf(requiredText) ==
                                -1
                            ) {
						...
						elementos.find(".card-body").each(function(){
                            ...
                            var oldParams = $this.find(
                                "span > .design_1 > div.parameters"
                            );
    ...
    $(document).on(
        ...
        function (e) {
            ...
            if (
                $(this).attr("value") == "" ||
                (!$(this).hasClass("negative-per-default") &&
                    $(this).attr("value") < 0)
            ) {
    ...
    function checkParamException(input) {
        ...
        if (exception !== undefined) {
            ...
            if (exception.type == "PARAM2") {
                var input2 = divParams.find(
                    'div[pid="' + exception.val + '"] input'
                );
    ...
    $("#modalSaveStrategy .modal-footer > .confirm-btn").on(
        "click",
        function () {
            let modalBody = $(this)
    ...
    function updateCalendarEndDate(ticket) {
        ...
        input.datepicker(
            "setEndDate",
            stringToDate(ticket.attr("end-date"), "/")
        );
            ...
            $.ajax({
                ...
                if (response.length > 20) {
                    if (auto_save !== "auto-save") {
                        ...
                        $("div.img-validate, .build-next").addClass(
                            "scnd_step"
                        );
                        $(
                            "div.img-validate img, .build-next, div.right-arrow img"
                        ).css("cursor", "pointer");
    ...
    $("div.img-build").on("click", function () {
        ...
        $("div.whizerd:eq(1)")
            .find("img")
            .attr("src", "images/build-active.png");
    ...
    $(document).on("click", ".scnd_step", function () {
        ...
        $(document).ajaxComplete(function (event, xhr, settings) {
            ...
            var error_0 =
                error_txt +
                " " +
                $("input.error_code_data").attr("data-error-0");
                ...
                if (
                    ...
                    if (validate_strategy($) == true) {
                        ...
                        if (!$this.hasClass("build-next")) {
                            ...
                            $(".summary-btn, div.img-download").addClass(
                                "thrd_step"
                            );
                            ...
                            if (old_value == new_value) {
                                ...
                                if (
                                    $.tooltipster.instances($(".build-next"))
                                        .length > 0
                                ) {
                                    $(".build-next").tooltipster("destroy");
                                }
                                ...
                                var session_id = $(
                                    "input[name=session_strategy_id]"
                                ).val();
            ...
            if (ajaxIndex == 1) {
                if (
                    ...
                    if (validate_strategy($) == true) {
                        ...
                        //if (old_value != new_value) {
                        if (
                            !$("button.build-next").hasClass("button_clicked")
                        ) {
                            $("#right_compile").removeAttr("style");
                            $('input[name="read_accept"]').attr(
                                "disabled",
                                true
                            );
                            $(
                                    ...
                                    $.ajax({
                                        ...
                                        .done(function (response) {
                                            ...
                                            if (response.length > 8) {
                                                ...
                                                $(
                                                    "#right_compile, .download_pay"
                                                ).css(
                                                    "pointer-events",
                                                    "unset"
                                                );
                                                $(
                                                    'input[name="read_accept"]'
                                                ).attr("disabled", false);
                                                $(
                                                    ".left-arrow, .img-build, .img-validate, .right-arrow"
                                                ).removeClass(
                                                ...
                                                if (
                                                    ...
                                                    console.log(
                                                        "Server not responding in 1"
                                                    );
                                                    $(".payment_loader").hide();
                                                    $(
                                                        "#right_compile, #right_download"
                                                    ).removeAttr("style");
                                                    $(
                                                        "#right_compile, #right_download"
                                                    ).css({
                    ...
                    if (settings.url.includes("save_data")) {
                        var error_txt = $("input.error_code_data").attr(
                            "data-error"
                        );
                        var error_0 =
                            error_txt +
    ...
    $("#validate-next").on("click", function () {
        ...
        $(".validate-tab .toggle-buttons").addClass("d-none");
        let graphCheckBox = $(
            '.toggle-buttons > label:first input[name="toggle"]'
        );
        ...
        function startChart() {
            ...
            $.ajax({
                    ...
                    toolTip: {
                        contentFormatter: function (e) {
                            ...
                            for (var i = 0; i < e.entries.length; i++) {
                                let { num, date, hour, profit, y } = e.entries[
                                    i
                                ].dataPoint;
                                if (num != 0) {
                                    content +=
                ...
                if (document.location.hostname == "test.tradeasy.tech") {
                    ...
                    console.log(
                        "No se reconoce el host:" + document.location.hostname
                    );
                }
                            ...
                            if (y_axix.length > 0) {
                                ...
                                for (var i = 0; i < y_axix.length; i++) {
                                    ...
                                    if (operationsDetail[i].tipoOP != -1) {
                                        let row = $("<tr></tr>");
                                        $(
                                            "#infoOperations table > tbody"
                                        ).append(row);
                                        row.append(
                                            "<td>" +
                                                y_axis_points.length +
                                                "</td>"
                                        );
                                        for (let key of keys) {
                                            row.append(
                                                "<td>" +
                                                    operationsDetail[i][key] +
                                                    "</td>"
                                            );
                                        }
                                        let splittedDate = operationsDetail[
                                            i
                                        ].fechaFin.split(" ");
                                ...
                                chart.options.axisX.interval = Math.round(
                                    x_interval
                                );
                                chart.render();
    ...
    $("#more-info").click(function () {
        ...
        function getTickerYearData(id, timeframe, year) {
            ...
            data.candles = data.candles.replace(
                /(\d)(?=(\d\d\d)+(?!\d))/g,
                "$1."
            );
            return data;
    ...
    setTimeout(function () {
        $(".card-header")
            ...
            .each(function () {
                var paramLabel = $(this).text();
                if (
                    $(this).parent().parent().parent().next().find("select")
                        .length != 0
                ) {
                    var paramValue = $(this)
                        .parent()
    ...
    let div = surveyModal.find(
        ".modal-body > .survey-second-step > div.d-none"
    );
    if (div.text() != null && div.text() != "") {
        ...
        surveyModal
            ...
            .text(survey.title);
        let surveyContainer = surveyModal.find(
            ".modal-body > .survey-second-step"
        );
    ...
    $("#nav-tabContent2 .omc.tab-pane").each(function (index, tabs) {
        ...
        $tabs.find(".trash").each(function (index, rows) {
            ...
            $rows.find("li").each(function (index, li) {
                ...
                var boolean_ =
                    $li.hasClass("sequence_li") &&
                    $li.next().hasClass("sequence_li");
                ...
                .done(function (response) {
                    ...
                    if (response_data != "") {
                        if (response_data.includes("complie_error")) {
                            ...
                            $("#right_compile").css({ opacity: 0.4 });
                            $("#right_compile").css("pointer-events", "none");
                            $('input[name="read_accept"]').removeAttr(
                                "checked"
                            );
                            $('input[name="agree_condition"]').removeAttr(
                                "checked"
                            );
                            $("#right_download").css({ opacity: 0.4 });
                            $("#right_download").css("pointer-events", "none");
                            ...
                            $('input[name="agree_condition"]').attr(
                                "disabled",
                                false
                            );
            ...
            if ($rows.find("li").length > 2) {
                $rows.find("li:not(.sign)").each(function (index, li) {
                    ...
                    if (
                        ...
                        $li.find(
                            '[data-template="true"] [data-field-type]'
                            ...
                            if (
                                ...
                                paramsData +=
                                    $param.find("input").attr("value") + ",";
                            } else if (
                                $param.attr("data-field-type") == "bool"
                            ) {
                        ...
                        elements +=
                            element_id + "," + paramsData.slice(0, -1) + ";";
                    }
                });
            }
                ...
                if (
                    ...
                    $("#modalLS button#confirmButton").click(function () {
                        ...
                        if (rows_open.length > 0) {
                            // Ocultar el cuadrado 'Arrastrar aquí'
                            if (rows_open[0].length > 2) {
                                $("#OPEN #trash > li.dashed_image_li").addClass(
                                    "display_none"
                                );
                            }
                            iterateRows($, rows_open, "OPEN", row_index);
                    ...
                    $('#modalLS button[data-dismiss="modal"]').click(
                        function () {
...
function addNumberOfRule(td, num) {
    ...
    var ruleTitle = $(
        '<th class="p-0"><p class="rule-title">' +
            text +
            " " +
            num +
            "</p></th>"
